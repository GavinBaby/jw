	    ...
	    if(FlowXML.value!=''){
		   ...
		   for ( var i = 0;i < Actions.childNodes.length;i++ ) {
               ...
               if(fromStep == MOVEOBJ.id || toStep == MOVEOBJ.id){                  
                  ...
                  {
                  	//直线
                  	//格式为：from="100,100" to="500,600"
                  	var ls_LinePointStr = getActionPoints(aType,document.getElementById(fromStep),document.getElementById(toStep));
				  					var lv_LinePointStr = ls_LinePointStr.split('from=');
				  					var lv_LinePointStr1 = lv_LinePointStr[1].split('to=');
				  	
				  					document.getElementById(aId).from.value = lv_LinePointStr1[0];
				  					document.getElementById(aId).to.value = lv_LinePointStr1[1];
                  }
                  else
                  {
                  	//折线
