   ...
   if(FlowXML.value!=''){
     ...
     var vmlHTML = '';
	   ...
	   for ( var i = 0;i < Steps.childNodes.length;i++ )
	   {
		   ...
		   if(nType=='BeginStep' || nType=='EndStep'){
		     ...
		     nStrokeColor = sStepColor;	
			   ...
			   nTextColor = Step.getElementsByTagName("VMLProperties").item(0).getAttribute("stepTextColor");
			   nStrokeColor = Step.getElementsByTagName("VMLProperties").item(0).getAttribute("stepStrokeColor");
			   if (nTextColor==null)
			   {
			   		nTextColor = StepTextColor;
			   }
			   if (nStrokeColor==null)
			   {
			   		nStrokeColor = StepColor;
			   }
			   
			   nColor1 = Step.getElementsByTagName("VMLProperties").item(0).getAttribute("stepColor1");
			   nColor2 = Step.getElementsByTagName("VMLProperties").item(0).getAttribute("stepColor2");
			   if (nColor1==null)
			   {
			   		nColor1 = StepColor1;
			   }
	   ...
	   for ( var i = 0;i < Actions.childNodes.length;i++ )
	   {
		   ...
		   lStrokeColor = Action.getElementsByTagName("VMLProperties").item(0).getAttribute("actionStrokeColor");
		   if (lStrokeColor==null)
			 {
			 		lStrokeColor = ActionColor;
			 }
...
function getStepHTML(stepType,z_index,id,text,textColor,strokeColor,shadowColor,isShadow,width,height,X,Y,strokeWeight,textWeight,color1,color2,is3D,step3DDepth){
   var stepHTML = '';
   var styleHTML = 'id='+id+' title="" style="z-index:'+z_index+';position:absolute;width:'+width+';height:'+height+';left:'+X+';top:'+Y+';" strokecolor="'+strokeColor+'" strokeweight="'+strokeWeight+'" onmouseover=\'return false;\' onmousedown=\'return false;\' oncontextmenu=\'return false;\' ';
...
function getActionHTML(actionType,z_index,id,actionText,fromStep,toStep,strokeColor,strokeWeight,startArrow,endArrow){
   ...
   if(actionType == 'Line'){
      actionHTML = '<v:line id='+id+' title="" style="z-index:'+z_index+';position:absolute;" '+getActionPoints(actionType,fromStep,toStep)+' strokecolor="'+strokeColor+'" strokeweight="'+strokeWeight+'" onmousedown=\'return false;\' oncontextmenu=\'return false;\'><v:stroke StartArrow="'+startArrow+'" EndArrow="'+endArrow+'"/></v:line>';
   }else{
      actionHTML = '<v:PolyLine id='+id+' title="" filled="false" Points="'+getActionPoints(actionType,fromStep,toStep)+'" style="z-index:'+z_index+';position:absolute;" strokecolor="'+strokeColor+'" strokeweight="'+strokeWeight+'" onmousedown=\'return false;\' oncontextmenu=\'return false;\'><v:stroke StartArrow="'+startArrow+'" EndArrow="'+endArrow+'"/></v:PolyLine>';
   }
...
function flowContextMenu(){
  //菜单源文本
  var menuSrc = '<menu><entity id="c0"><description>新建环节</description><image>/jwwebflow/inc/contextMenu/images/add_step.gif</image><fontcolor>'+MenuTextColor_enable+'</fontcolor><onClick>cleancontextMenu();newStep("NormalStep");</onClick><contents></contents></entity>';
  menuSrc+= '<entity id="c1"><description>新建路由</description><image>/jwwebflow/inc/contextMenu/images/add_action.gif</image><fontcolor>'+MenuTextColor_enable+'</fontcolor><onClick>cleancontextMenu();newAction();</onClick><contents></contents></entity>';
  menuSrc+= '<entity id="c2"><description>刷新</description><image>/jwwebflow/inc/contextMenu/images/refresh_vml.gif</image><fontcolor>'+MenuTextColor_enable+'</fontcolor><onClick>cleancontextMenu();redrawVML();</onClick><contents></contents></entity></menu>';
...
function stepContextMenu(stepId,stepType){
  ...
  if(stepType!='BeginStep' && stepType!='EndStep')
  {
  	menuSrc += '<entity id="c0"><description>修改环节['+stepId+']</description><image>/jwwebflow/inc/contextMenu/images/edit_obj.gif</image><fontcolor>'+MenuTextColor_enable+'</fontcolor><onClick>cleancontextMenu();editStep("'+stepId+'","'+stepType+'");</onClick><contents></contents></entity>';
  	menuSrc+= '<entity id="c1"><description>删除环节['+stepId+']</description><image>/jwwebflow/inc/contextMenu/images/del_obj.gif</image><fontcolor>'+MenuTextColor_enable+'</fontcolor><onClick>cleancontextMenu();delStep("'+stepId+'");</onClick><contents></contents></entity>';
  }
  menuSrc+= '<entity id="c2"><description>刷新</description><image>/jwwebflow/inc/contextMenu/images/refresh_vml.gif</image><fontcolor>'+MenuTextColor_enable+'</fontcolor><onClick>cleancontextMenu();redrawVML();</onClick><contents></contents></entity></menu>';
  showContextMenu(menuSrc);
}
...
function actionContextMenu(actionId){
  //菜单源文本
  var menuSrc = '<menu><entity id="c0"><description>修改路由['+actionId+']</description><image>/jwwebflow/inc/contextMenu/images/edit_obj.gif</image><fontcolor>'+MenuTextColor_enable+'</fontcolor><onClick>cleancontextMenu();editAction("'+actionId+'");</onClick><contents></contents></entity>';
  menuSrc+= '<entity id="c1"><description>删除路由['+actionId+']</description><image>/jwwebflow/inc/contextMenu/images/del_obj.gif</image><fontcolor>'+MenuTextColor_enable+'</fontcolor><onClick>cleancontextMenu();delAction("'+actionId+'");</onClick><contents></contents></entity>';
  menuSrc+= '<entity id="c2"><description>刷新</description><image>/jwwebflow/inc/contextMenu/images/refresh_vml.gif</image><fontcolor>'+MenuTextColor_enable+'</fontcolor><onClick>cleancontextMenu();redrawVML();</onClick><contents></contents></entity></menu>';
  showContextMenu(menuSrc);
}
...
function showContextMenu(menuSrc){
  ...
  loadContextMenu(menuText,'/jwwebflow/inc/contextMenu/contextmenu.xsl')
}
...
function workFlowDialog(id,type){
	...
	switch (type){
        case 'NormalStep': dialogURL = id==null?'/jwwebflow/_stepdialog.html':'/jwwebflow/_stepdialog.html?stepid='+id;break;
	    case 'BeginStep': dialogURL = id==null?'/jwwebflow/_stepdialog.html':'/jwwebflow/_stepdialog.html?stepid='+id;break;
	    case 'EndStep': dialogURL = id==null?'/jwwebflow/_stepdialog.html':'/jwwebflow/_stepdialog.html?stepid='+id;break;
	    case 'Action': dialogURL = id==null?'/jwwebflow/_actiondialog.html':'/jwwebflow/_actiondialog.html?actionid='+id;break;
		case 'Flow': dialogURL = id==null?'/jwwebflow/_flowdialog.html':'/jwwebflow/_flowdialog.html?flowid='+id;break;		
	}
