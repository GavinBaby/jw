...
//				w = width - leftWidth - (hasScroll == true ? 28 : 10);
	...
	function makeDraggable(target, panel){
		...
		panel.panel('panel').draggable({
			...
			onStopDrag:function(e){
				...
				spacer.remove();
				//setSize(target);//lisy del 130426
				panel.panel('move');
				
				var opts = $.data(target, 'portal').options;
				opts.onStateChange.call(target);
				setTop(target);//lisy add 130201				
			}
		});
	...
	function setTop(target){
		var t = $(target);
		var spacerlen = t.find('div.portal-spacer').length;
		if (spacerlen != 0){
		  for (var i=0;i<spacerlen;i++)
		  {
			var c = t.find('div.portal-spacer:eq(' + i + ')');
			c.remove();
		  }
		}
	}
